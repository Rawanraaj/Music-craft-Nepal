...
const MEGA_MENU_CATEGORIES = [
  { label: 'Guitars', path: '/shop?category=Guitars', icon: Guitar, desc: 'Acoustic, electric, classical & ukuleles' },
  { label: 'Traditional Instruments', path: '/shop?category=Traditional+Instruments', icon: Music, desc: 'Sarangi, tungna, damphu & more' },
  { label: 'Keyboards', path: '/shop?category=Keyboards', icon: Piano, desc: 'Harmoniums & portable keyboards' },
  { label: 'Percussion', path: '/shop?category=Percussion', icon: Drum, desc: 'Madal, tabla, dholak & jhyali' },
  { label: 'String Instruments', path: '/shop?category=String+Instruments', icon: Music2, desc: 'Sitar, tanpura & classical strings' },
  { label: 'Wind Instruments', path: '/shop?category=Wind+Instruments', icon: Wind, desc: 'Bansuri flutes & folk brass' },
  { label: 'Accessories', path: '/shop?category=Accessories', icon: Headphones, desc: 'Strings, capos, bows & parts' },
];

const TOP_NAV_LINKS = [
  { label: 'Home', path: '/', icon: Home },
  { label: 'What\'s New', path: '/shop?sort=newest', icon: Sparkles },
  { label: 'Deals', path: '/shop?deals=true', icon: Tag },
  { label: 'Wholesale', path: '/wholesale', icon: Package },
  { label: 'Articles & Tips', path: '/articles', icon: Newspaper },
  { label: 'Contact', path: '/contact', icon: Mail },
];

const MOBILE_LINKS: { label: string; path: string; isHeader?: boolean }[] = [
  { label: 'Home', path: '/' },
  { label: 'Shop By Category', path: '/shop', isHeader: true },
  ...MEGA_MENU_CATEGORIES.map((c) => ({ label: c.label, path: c.path })),
  { label: 'What\'s New', path: '/shop?sort=newest', isHeader: true },
  { label: 'Deals', path: '/shop?deals=true' },
  { label: 'Wholesale', path: '/wholesale' },
  { label: 'Articles & Tips', path: '/articles' },
      ...
      <div className="bg-mcn-dark text-white text-xs hidden md:block">
        <div className="max-w-7xl mx-auto px-4 flex items-center justify-between h-9">
          ...
          <div className="flex items-center gap-4">
            <Link to="/about" className="hover:text-mcn-mint transition-colors">Our Story</Link>
            <Link to="/wholesale" className="hover:text-mcn-mint transition-colors">Wholesale</Link>
            <Link to="/contact" className="hover:text-mcn-mint transition-colors">Contact</Link>
          </div>
        </div>
      </div>
